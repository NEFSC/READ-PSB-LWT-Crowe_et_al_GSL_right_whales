...
res_all_bar<-ggplot(res_all, mapping = aes(x = jday, y = SightingYear)) + 
  ...
  scale_x_continuous(breaks = c(121,153,183,214,245,275,306,336), labels = c("May","June","July","Aug","Sept","Oct","Nov","Dec"), limits = c(121,354), name = "Month")+
  ylab("")+
   theme(panel.grid.minor = element_blank(),
         legend.position="right",
...
ggsave(filename = 'Fig5A.svg',behvfig,device = 'svg', './Figures', width = 90, height = 154, units = "mm", dpi = 320)
```
...
cap_colors = c("< 5 days" = wes_palette("Darjeeling1")[1], "5+ days" = wes_palette("Darjeeling1")[2], "No ID" = "black")
...
#####
fig6<-ggpubr::ggarrange(indi90,rastmap,labels = "AUTO", nrow = 2, align = "hv")
...
```{r table_output, echo=FALSE, warning=FALSE, message=FALSE}
table1
table2
table3
table4
...
sup_map<-ggplot()+
  ...
  coord_sf(xlim = c(-70,-57), ylim = c(45.5,51.5), crs = 4269)+
  scale_fill_manual(values = sup_map_fill)+
  scale_color_manual(values = sup_map_fill, guide = F)
...
ggsave(filename = 'FigS1A.png',figS1A,device = 'png', './Figures', width = 169, height = 160, units = "mm", dpi = 320)
...
```{r GOF, echo=T, warning=FALSE, message=FALSE}
