...
abund<-function(x){
  x%>%
  mutate(Estimated = round(Observed/p,0),
         Lower = round(Observed/ucl),
         Upper = round(Observed/lcl))%>%
  dplyr::select(Block, Dates, Observed, Estimated, Lower, Upper, p, SE)
}
...
```
Fig. S2. Model averaged estimates of survival probability across all years (2015–2019). Note that the last occasion is confounded.
